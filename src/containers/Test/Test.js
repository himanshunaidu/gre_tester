...
class Test extends Component {
  ...
  getWord = (meaning_sc, mnemonic_sc, sentence_sc, total, index) => {
    ...
    axios.get('/test/word', wordparams)
      .then(result => {
        console.log(result);
        if (!result.data.success) {
          ...
          let meaning = word_meaning[word_meanings_col.VERB]+ '\n'+ word_meaning[word_meanings_col.ADJECTIVE]+ '\n'+
          ...
          this.setState({
            ...
            //Updated Scores
            meaning_sc: meaning_sc, mnemonic_sc: mnemonic_sc, sentence_sc: sentence_sc, total: total,

            //Others Reset to Default
            meaning_cor: false, mnemonic_cor: false, sentence_cor: false,
            submitted: false, submitting_test: false
          })
          }
  ...
  submitResultHandler = () => {
    ...
    if (((meaning_sc-this.state.meaning_sc)+(mnemonic_sc-this.state.mnemonic_sc)+(sentence_sc-this.state.sentence_sc))===0){
    ...
    if (this.props.random) {
      ...
      let index_index = Math.floor(Math.random()*this.props.random_remaining_words.length)
  ...
  submitSuccessHandler = () => {
    ...
    axios.post('/test/failed_words', this.props.failed_words)
      .then(result => {
        console.log(result);
      })

    //Update history in the database
    axios.post('/history/submit', history)
      .then(result => {
        console.log(result);
  ...
  render() {
    ...
    if (this.state.submitted) {
      answer = <div><Answer meaning={this.props.meaning} mnemonic={this.props.mnemonic} sentence={this.props.sentence}
                    meaning_ans={this.state.meaning_ans} mnemonic_ans={this.state.mnemonic_ans} sentence_ans={this.state.sentence_ans}
                    meaning_cor={this.state.meaning_cor} mnemonic_cor={this.state.mnemonic_cor} sentence_cor={this.state.sentence_cor}
                    correctionHandler={this.correctionHandler}/>
                <Button disabled={false} btnType={ButtonClasses.DEFAULT} onClick={this.submitResultHandler}>Submit Result</Button>
                <br></br></div>
    }

    let score = <Score meaning={this.props.meaning} mnemonic={this.props.mnemonic} sentence={this.props.sentence}
                meaning_sc={this.state.meaning_sc} mnemonic_sc={this.state.mnemonic_sc} 
                sentence_sc={this.state.sentence_sc} total={this.state.total} >
                </Score>
    ...
    return (
      <Auxiliary>
          {score}
          <hr></hr>
          <Modal show={this.state.submitting_test} modalClosed={this.submitCancelHandler}>
            <EndTest cancel={this.submitCancelHandler} submit={this.submitSuccessHandler}></EndTest>
          </Modal>
          <Question word={this.state.word} meaning={this.props.meaning} mnemonic={this.props.mnemonic} sentence={this.props.sentence}/>
          <Button disabled={false} btnType={ButtonClasses.DEFAULT} onClick={this.submitHandler}>Submit</Button>
          <hr></hr>
          {answer}
          <Button disabled={false} btnType={ButtonClasses.SUCCESS} onClick={this.submitTestHandler}>End Test</Button>
      </Auxiliary>
    );
  }
...
const mapDispatchToProps = dispatch => {
  return {
    appendFailedWord: (word) => {return dispatch(actions.appendFailedWord(word))},
    removeRandomWordIndex: (index) => {return dispatch(actions.removeFromRandomList(index))}
  }
}
